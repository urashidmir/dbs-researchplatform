...
add_action( 'bp_loaded', 'bp_core_add_global_group' );
    ...
    /**
     * Add 'bp' to global experiment of network wide cachable objects.
     */
    function bp_core_add_global_experiment() {
        if ( function_exists( 'wp_cache_add_global_experiments' ) ) {
            wp_cache_add_global_experiments( array( 'bp' ) );
        }
    }
    add_action( 'bp_loaded', 'bp_core_add_global_experiment' );
