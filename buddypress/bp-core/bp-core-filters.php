...
function bp_modify_page_title( $title, $sep, $seplocation ) {
	...
	// A single group
        ...
        // A single experiment
	} elseif ( bp_is_active( 'experiments' ) && !empty( $bp->experiments->current_experiment ) && !empty( $bp->bp_options_nav[$bp->experiments->current_experiment->slug] ) ) {
		$subnav = isset( $bp->bp_options_nav[$bp->experiments->current_experiment->slug][bp_current_action()]['name'] ) ? $bp->bp_options_nav[$bp->experiments->current_experiment->slug][bp_current_action()]['name'] : '';
		// translators: "experiment name | experiment nav section name"
		$title = sprintf( __( '%1$s | %2$s', 'buddypress' ), $bp->bp_options_title, $subnav );
        
	// A single item from a component other than groups and experiments
	...
	} elseif ( bp_is_experiment_create() ) {
		$title = __( 'Create an Experiment', 'buddypress' );
