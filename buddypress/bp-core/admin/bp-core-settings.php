...
/**
    * Experiments settings section description for the settings page
     *
     * @since BuddyPress (1.6)
*/
function bp_admin_setting_callback_experiments_section() { }
    
    /**
     * Allow all users to create experiment field
     *
     * @since BuddyPress (1.6)
     *
     * @uses checked() To display the checked attribute
     */
    function bp_admin_setting_callback_experiment_creation() {
        ?>

<input id="bp_restrict_experiment_creation" name="bp_restrict_experiment_creation" type="checkbox"value="1" <?php checked( !bp_restrict_experiment_creation( false ) ); ?> />
<label for="bp_restrict_experiment_creation"><?php _e( 'Enable experiment creation for all users', 'buddypress' ); ?></label>
<p class="description"><?php _e( 'Administrators can always create experiments, regardless of this setting.', 'buddypress' ); ?></p>
...
function bp_core_admin_settings_save() {
	...
	if ( isset( $_GET['page'] ) && 'bp-settings' == $_GET['page'] && !empty( $_POST['submit'] ) ) {
		...
		$legacy_options = array(
            ...
            'bp_restrict_experiment_creation',
