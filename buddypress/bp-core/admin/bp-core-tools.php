...
function bp_admin_repair_list() {
	...
	// Experiments:
	// - user experiment count
	if ( bp_is_active( 'experiments' ) ) {
		$repair_list[10] = array(
                                 'bp-experiment-count',
                                 __( 'Count expriments for each user', 'buddypress' ),
                                 'bp_admin_repair_experiment_count',
                                 );
	}
...
function bp_admin_repair_group_count() {
	...
	return array( 0, sprintf( $statement, __( 'Complete!', 'buddypress' ) ) );
        ...
        /**
         * Recalculate experiment counts for each user.
         *
         * @since BuddyPress (2.0.0)
         *
         * @return array
         */
        function bp_admin_repair_experiment_count() {
            global $wpdb, $bp;
            ...
            $statement = __( 'Counting the number of experiments for each user&hellip; %s', 'buddypress' );
            $result    = __( 'Failed!', 'buddypress' );
            
            $sql_delete = "DELETE FROM {$wpdb->usermeta} WHERE meta_key IN ( 'total_experiment_count' );";
            if ( is_wp_error( $wpdb->query( $sql_delete ) ) ) {
                return array( 1, sprintf( $statement, $result ) );
            }
            ...
            if ( $total_users > 0 ) {
                ...
                $offset = 0;
                while ( $offset < $total_users ) {
                    // But only bother to update counts for users that have experiments
                    $users = $wpdb->get_col( $wpdb->prepare( "SELECT user_id FROM wp_bp_experiments_members WHERE is_confirmed = 1 AND is_banned = 0 AND user_id > %d AND user_id <= %d", $offset, $offset + $per_query ) );
                    
                    foreach ( $users as $user ) {
                        BP_Experiments_Member::refresh_total_experiment_count_for_user( $user );
                    }
...
function bp_core_admin_available_tools_intro() {
	...
	<div class="tool-box">
		...
		<p>
			<?php esc_html_e( 'BuddyPress keeps track of various relationships between users, groups, experiments, and activity items. Occasionally these relationships become out of sync, most often after an import, update, or migration.', 'buddypress' ); ?>
