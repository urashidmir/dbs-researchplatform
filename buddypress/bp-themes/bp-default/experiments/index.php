...
/**
 * BuddyPress - Experiments Directory
 *
 * @package BuddyPress
...
get_header( 'buddypress' ); ?>
	...
	<div id="content">
		...
		<form action="" method="post" id="experiments-directory-form" class="dir-form">
			...
			<h3><?php _e( 'Experiments Directory', 'buddypress' ); ?><?php if ( is_user_logged_in() && bp_user_can_create_experiments() ) : ?> &nbsp;<a class="button" href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_experiments_root_slug() . '/create' ); ?>"><?php _e( 'Create an Experiment', 'buddypress' ); ?></a><?php endif; ?></h3>
			...
			<div class="item-list-tabs" role="navigation">
				<ul>
					<li class="selected" id="experiments-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_experiments_root_slug() ); ?>"><?php printf( __( 'All Experiments <span>%s</span>', 'buddypress' ), bp_get_total_experiment_count() ); ?></a></li>

					<?php if ( is_user_logged_in() && bp_get_total_experiment_count_for_user( bp_loggedin_user_id() ) ) : ?>

						<li id="experiments-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_experiments_slug() . '/my-experiments' ); ?>"><?php printf( __( 'My Experiments <span>%s</span>', 'buddypress' ), bp_get_total_experiment_count_for_user( bp_loggedin_user_id() ) ); ?></a></li>
			...
			<div id="experiments-dir-list" class="experiments dir-list">
				...
				<?php locate_template( array( 'experiments/experiments-loop.php' ), true ); ?>
