...
class BP_Legacy extends BP_Theme_Compat {
	...
	protected function setup_actions() {
		...
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			...
			if ( bp_is_active( 'experiments' ) ) {
				add_action( 'bp_experiment_header_actions',     'bp_experiment_join_button',           5 );
				add_action( 'bp_experiment_header_actions',     'bp_experiment_new_topic_button',      20 );
				add_action( 'bp_directory_experiments_actions', 'bp_experiment_join_button' );
				add_filter( 'bp_experiments_directory_header',  'bp_legacy_theme_experiment_create_button' );
				//add_filter( 'bp_blogs_directory_header',   'bp_legacy_theme_blog_create_button' );
			}
		...
		$actions = array(
            ...
            'experiments_filter'   => 'bp_legacy_theme_object_template_loader',
            ...
            'invite_filter_experiment'   => 'bp_legacy_theme_invite_template_loader_experiment',
            'requests_filter_experiment' => 'bp_legacy_theme_requests_template_loader_experiment',
            ...
            // Experiments
            'experiments_invite_user' => 'bp_legacy_theme_ajax_invite_user_experiment',
            'joinleave_experiment'    => 'bp_legacy_theme_ajax_joinleave_experiment',
	...
	public function enqueue_scripts() {
		...
		$params = apply_filters( 'bp_core_get_js_strings', array(
            ...
            'leave_experiment_confirm' => __( 'Are you sure you want to leave this experiment?', 'buddypress' ),
...
function bp_legacy_theme_group_create_button( $title ) {
	return $title . ' ' . bp_get_group_create_button();
        ...
        /**
         * Add the Create a Experiment button to the Experiments directory title.
         *
         * bp-legacy puts the Create a Experiment button into the page title, to mimic
         * the behavior of bp-default.
         *
         * @since BuddyPress (2.0.0)
         *
         * @param string $title Experiments directory title.
         * @return string
         */
        function bp_legacy_theme_experiment_create_button( $title ) {
            return $title . ' ' . bp_get_experiment_create_button();
        }
...
function bp_legacy_theme_invite_template_loader() {
	...
	exit();
        ...
        /**
         * Load experiment invitations loop to handle pagination requests sent via AJAX.
         *
         * @since BuddyPress (2.0.0)
         */
        function bp_legacy_theme_invite_template_loader_experiment() {
            bp_get_template_part( 'experiments/single/invites-loop' );
            exit();
        }
...
function bp_legacy_theme_requests_template_loader() {
	...
	exit();
        ...
        /**
         * Load experiment membership requests loop to handle pagination requests sent via AJAX.
         *
         * @since BuddyPress (2.0.0)
         */
        function bp_legacy_theme_requests_template_loader_experiment() {
            bp_get_template_part( 'experiments/single/requests-loop' );
            exit();
        }
...
function bp_legacy_theme_activity_template_loader() {
	...
	switch ( $scope ) {
		...
		case 'groups':
			...
			break;
		case 'experiments':
			$feed_url = bp_loggedin_user_domain() . bp_get_activity_slug() . '/experiments/feed/';
			break;
		case 'favorites':
...
function bp_legacy_theme_post_update() {
    ...
    } elseif ( $_POST['object'] == 'experiments' ) {
		if ( ! empty( $_POST['item_id'] ) && bp_is_active( 'experiments' ) )
			$activity_id = experiments_post_update( array( 'content' => $_POST['content'], 'experiment_id' => $_POST['item_id'] ) );
...
function bp_legacy_theme_ajax_invite_user() {
	...
	if ( 'invite' == $_POST['friend_action'] ) {
        ...
        /**
         * Invites a friend to join an experiment via a POST request.
         *
         * @since BuddyPress (1.2)
         * @todo Audit return types
         */
        function bp_legacy_theme_ajax_invite_user_experiment() {
            // Bail if not a POST action
            if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) )
                return;
            
            check_ajax_referer( 'experiments_invite_uninvite_user' );
            
            if ( ! $_POST['friend_id'] || ! $_POST['friend_action'] || ! $_POST['experiment_id'] )
                return;
            
            if ( ! bp_experiments_user_can_send_invites( $_POST['experiment_id'] ) )
                return;
            ...
            if ( 'invite' == $_POST['friend_action'] ) {
                $experiment = experiments_get_experiment( $experiment_id );
                
                // Users who have previously requested membership do not need
                // another invitation created for them
                if ( BP_Experiments_Member::check_for_membership_request( $friend_id, $experiment_id ) ) {
                    $user_status = 'is_pending';
                    
                    // Create the user invitation
                } else if ( experiments_invite_user( array( 'user_id' => $friend_id, 'experiment_id' => $experiment_id ) ) ) {
                    $user_status = 'is_invited';
                ...
                $uninvite_url = bp_is_current_action( 'create' ) ? bp_get_root_domain() . '/' . bp_get_experiments_root_slug() . '/create/step/experiment-invites/?user_id=' . $friend_id : bp_get_experiment_permalink( $experiment ) . 'send-invites/remove/' . $friend_id;
                ...
                echo '<span class="activity">' . esc_attr( $user->last_active ) . '</span>';
                echo '<div class="action">
				<a class="button remove" href="' . wp_nonce_url( $uninvite_url, 'experiments_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'buddypress' ) . '</a>
                </div>';
                
                if ( 'is_pending' == $user_status ) {
                    echo '<p class="description">' . sprintf( __( '%s has previously requested to join this experiment. Sending an invitation will automatically add the member to the experiment.', 'buddypress' ), $user->user_link ) . '</p>';
                }
                ...
                // Remove the unsent invitation
                if ( ! experiments_uninvite_user( $friend_id, $experiment_id ) ) {
                    return;
                }
...
function bp_legacy_theme_ajax_joinleave_group() {
	...
	exit;
        ...
        /**
         * Join or leave an experiment when clicking the "join/leave" button via a POST request.
         *
         * @return string HTML
         * @since BuddyPress (1.2)
         */
        function bp_legacy_theme_ajax_joinleave_experiment() {
            // Bail if not a POST action
            if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) )
                return;
            
            // Cast gid as integer
            $experiment_id = (int) $_POST['gid'];
            
            if ( experiments_is_user_banned( bp_loggedin_user_id(), $experiment_id ) )
                return;
            
            if ( ! $experiment = experiments_get_experiment( array( 'experiment_id' => $experiment_id ) ) )
                return;
            
            if ( ! experiments_is_user_member( bp_loggedin_user_id(), $experiment->id ) ) {
                if ( 'public' == $experiment->status ) {
                    check_ajax_referer( 'experiments_join_experiment' );
                    
                    if ( ! experiments_join_experiment( $experiment->id ) ) {
                        _e( 'Error joining experiment', 'buddypress' );
                    } else {
                        echo '<a id="experiment-' . esc_attr( $experiment->id ) . '" class="leave-experiment" rel="leave" title="' . __( 'Leave Experiment', 'buddypress' ) . '" href="' . wp_nonce_url( bp_get_experiment_permalink( $experiment ) . 'leave-experiment', 'experiments_leave_experiment' ) . '">' . __( 'Leave Experiment', 'buddypress' ) . '</a>';
                    }
                    
                } elseif ( 'private' == $experiment->status ) {
                    
                    // If the user has already been invited, then this is
                    // an Accept Invitation button
                    if ( experiments_check_user_has_invite( bp_loggedin_user_id(), $experiment->id ) ) {
                        check_ajax_referer( 'experiments_accept_invite' );
                        
                        if ( ! experiments_accept_invite( bp_loggedin_user_id(), $experiment->id ) ) {
                            _e( 'Error requesting membership', 'buddypress' );
                        } else {
                            echo '<a id="experiment-' . esc_attr( $experiment->id ) . '" class="leave-experiment" rel="leave" title="' . __( 'Leave Experiment', 'buddypress' ) . '" href="' . wp_nonce_url( bp_get_experiment_permalink( $experiment ) . 'leave-experiment', 'experiments_leave_experiment' ) . '">' . __( 'Leave Experiment', 'buddypress' ) . '</a>';
                        }
                        ...
                        check_ajax_referer( 'experiments_request_membership' );
                        
                        if ( ! experiments_send_membership_request( bp_loggedin_user_id(), $experiment->id ) ) {
                            _e( 'Error requesting membership', 'buddypress' );
                        } else {
                            echo '<a id="experiment-' . esc_attr( $experiment->id ) . '" class="membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'buddypress' ) . '" href="' . bp_get_experiment_permalink( $experiment ) . '">' . __( 'Membership Requested', 'buddypress' ) . '</a>';
                        }
                    }
                }
                
            } else {
                check_ajax_referer( 'experiments_leave_experiment' );
                
                if ( ! experiments_leave_experiment( $experiment->id ) ) {
                    _e( 'Error leaving experiment', 'buddypress' );
                } elseif ( 'public' == $experiment->status ) {
                    echo '<a id="experiment-' . esc_attr( $experiment->id ) . '" class="join-experiment" rel="join" title="' . __( 'Join Experiment', 'buddypress' ) . '" href="' . wp_nonce_url( bp_get_experiment_permalink( $experiment ) . 'join', 'experiments_join_experiment' ) . '">' . __( 'Join Experiment', 'buddypress' ) . '</a>';
                } elseif ( 'private' == $experiment->status ) {
                    echo '<a id="experiment-' . esc_attr( $experiment->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'buddypress' ) . '" href="' . wp_nonce_url( bp_get_experiment_permalink( $experiment ) . 'request-membership', 'experiments_send_membership_request' ) . '">' . __( 'Request Membership', 'buddypress' ) . '</a>';
                }
            }
